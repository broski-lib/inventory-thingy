...
import { getTagsForItems } from "./tags.server"
import { getBatchesForItems } from "./batches.server"
import type {
  InventoryItem,
...
/**
 * Plain query for one page of items. The total is computed in the same
 * round trip via a window function, avoiding a separate COUNT query over
 * the full filter.
 */
...
/** Most-frequent location across items and batches. */
export async function fetchMostCommonLocation(
  orgId: string
): Promise<string | null> {
  const db = getDb()
  const [itemRows, batchRows] = await Promise.all([
...
export async function fetchItemByQrCode(
  orgId: string,
  qrCode: string
...
export async function uploadItemImageImpl(
  form: FormData,
  orgId: string
 ...
 * Both queries run in parallel. Returns null if the item is not found
 * in the org (callers should use `notFound()` to surface a 404).
 */
export async function fetchItemWithHistory(
  orgId: string,
  id: string
): Promise<{ item: typeof items.$inferSelect; logs: ActivityLog[] } | null> {
  const db = getDb()
 ...
 */
export async function bulkDeleteItemsImpl(
  ids: string[],
  userId: string,
  orgId: string,
  has: HasFn
 ...
 */
export async function bulkUpdateStatusImpl(
  ids: string[],
  status: ItemStatus,
  userId: string,
  orgId: string,
 ...
 */
export async function bulkUpdateLocationImpl(
  ids: string[],
  location: string,
  userId: string,
  orgId: string,
