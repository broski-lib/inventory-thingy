...
/**
 * Fetch tags for a set of items in one query. Internal helper — not a
 * server fn. Returns a Map of itemId → tags (ordered by tag name).
 */
...
export const setItemTags = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data: { itemId, tagIds }, context }) => {
    const { orgId, has } = context
    const db = getDb()
